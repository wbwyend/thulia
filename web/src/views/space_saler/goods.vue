<template>
    <div style="width: 100%; margin-top: 20px;">
        <el-row style="height: 40px; width: 100%;">
            <div style="text-align: center; font-size: 24px;">在售商品管理</div>
        </el-row>
                                        ...
                                        <el-main style="padding: 0; display: flex;">
                                            <div style="flex: 3;">
                                                <el-upload :action="$baseUrl + '/file/upload/picture/goods'"
                                                    :show-file-list="false" :headers="{ token: saler.token }"
...
export default {
    ...
    mounted() {
        this.$request.post('/goods/get/saler', this.saler.id).then(res => {
            if (res.code === '200') {
    ...
    methods: {
        ...
        batchDelete() {
            if (this.multipleSelection.length === 0) {
                ...
                this.$confirm('此操作将下架所选商品, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    this.$request.post('/goods/batch/delete/shop', this.multipleSelection).then(res => {
                        if (res.code === '200') {
                            this.$message({
                                type: 'success',
                                message: '操作成功!'
        ...
        addGoods() {
            this.$refs['addRef'].validate((valid) => {
                if (this.imageUrl === '') {
                    this.$message.error('商品图片为空');
                } else {
                    if (valid) {
                        console.log(this.form.cid);
                        this.form.picture = this.imageUrl;
                        this.form.sid = this.saler.sid;
                        this.form.sales = '0';
                        this.form.saler = this.saler.id;
                        this.$request.post('/goods/add/shop', this.form).then(res => {
                            if (res.code === '200') {
        ...
        handleDelete(index, row) {
            this.$confirm('此操作将下架该商品, 是否继续?', '提示', {
                ...
                this.$request.post('/goods/delete/shop', row).then(res => {
                    if (res.code === '200') {
                        this.$message({
                            type: 'success',
                            message: '操作成功!'
        ...
        editGoods() {
            this.$refs['editRef'].validate((valid) => {
                ...
                if (valid) {
                    ...
                    this.form.picture = this.imageUrl;
                    this.form.sid = this.saler.sid;
                    this.form.onsale = 'true';

                    this.$request.post('/goods/update/saler', this.form).then(res => {
                        if (res.code === '200') {
